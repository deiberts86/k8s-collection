...
5. Now to the installation of NGINX-Ingress as a one-liner:
    ...
    --set controller.ingressClass="nginx-secondary" \
    --set controller.ingressClassResource.default=false \
    --set controller.ingressClassResource.name="nginx-secondary" \
    --set controller.service.type=LoadBalancer \
    --set controller.kind=DaemonSet \
    --set controller.config.use-forwarded-headers=true \
    --set controller.config.extraArgs.enable-ssl-passthrough=true \
    --namespace nginx \
    --create-namespace
    ```

6. Or create your own values.yaml file for easier deployment. Note, you can add more values as needed. Examples provided in the `helm-values-example` folder in this repo.

7. Create your `nginx-values.yaml` file.
...
8. Now to use your new values file.
...
9. Once successful, you should have a dedicated NGINX ingressClass just for SSL PASSTHROUGH and forward headers.
...
10. If your checks all pass and you have a valid external-IP for your service, you should now be able to use this `nginx-secondary` ingressClass.
